    ...
    async started() {
        ...
        try {
            ...
            if (devices.length > 0) {
                ...
                this.logger.info(`Testing manual heartbeat for device ${testDevice.ieeeAddr}`);
                const heartbeatResult = await this.broker.call("zigbee.sendHeartbeat", {
                    deviceId: testDevice.ieeeAddr,
                    status: "test_heartbeat"
                });
                this.logger.info("Manual heartbeat result:", heartbeatResult);
            }
